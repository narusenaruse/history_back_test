...
public class FormMatcher extends TypeSafeMatcher<Object> {
	 ...
	 * @param expected 期待値
	 */
	public FormMatcher(Object expected) {
		this.expected = expected;
	}

	/**
	 * オブジェクトを検証します。
	 * オブジェクトの中身は、メモリ比較でないものとList<独自クラス>であれば検証可能。
	 */
	@Override
	protected boolean matchesSafely(Object actual) {
		...
		//オブジェクトの各値の取得
		List<Object> actualValueList = getObjectValue(actual);
		List<Object> expectedValueList = getObjectValue(expected);
		...
		for(int i = 0; i < expectedValueList.size(); i++) {
			...
			//nullチェック
			if(!Objects.isNull(expectedValueList.get(i))) {

				//期待値と実際値を比較し、間違っている場合にtrue
				if(!actualValueList.get(i).equals(expectedValueList.get(i))) {
					succeedFlg = succeedCheck(actualValueList.get(i), expectedValueList.get(i));
				}
				...
				//expectedValueList.get(i)がnullでactualValueList.get(i)に値が入っている場合、即座にfalseを返却する
				if(!Objects.isNull(actualValueList.get(i))) {
					succeedFlg = false;
					return succeedFlg;
				}
			}
